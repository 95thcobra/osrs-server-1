...
public abstract class RouteStrategy {
	 ...
	 */
	public abstract int getApproxDestinationZ();
	 ...
	 */
	public abstract int getApproxDestinationSizeZ();
	...
	protected static boolean checkFilledRectangularInteract(int[][] clip, int currentX, int currentY, int sizeX, int sizeY, int targetX, int targetY, int targetSizeX, int targetSizeY, int accessBlockFlag) {
		...
		if (destEndX == currentX && (accessBlockFlag & 0x2) == 0) { // can we enter from east ?
			...
			int curClipX = currentX > targetX ? currentX : targetX;

			for (int clipEnd = srcEndX < destEndX ? srcEndX : destEndX; curClipX < clipEnd; curClipX++) {
				if (((clip[curClipX][targetY]) & 0x20) == 0)
					return true;
			}
		}
		return false;
	...
	protected static boolean checkFilledRectangularInteractCombat(int[][] clip, int currentX, int currentY, int sizeX, int sizeY, int targetX, int targetY, int targetSizeX, int targetSizeY, int accessBlockFlag) {
		// TODO refactor
		/*int srcEndX = currentX + sizeX;
		int srcEndY = currentY + sizeY;
		int destEndX = targetX + targetSizeX;
		...
		if (destEndX == currentX && (accessBlockFlag & 0x2) == 0) { // can we enter from east ?
			...
			int targx = currentX > targetX ? currentX : targetX;
			for (int curx = srcEndX < destEndX ? srcEndX : destEndX; targx <= curx; targx++) {
				System.out.println("passed?? " + targx + ". " + srcEndX +", " + srcEndY + ", " + destEndX+", " + destEndY + ", " + currentX + ", " + currentY + ", " + sizeX + ", " + sizeY + ", " + targetX + ", " + targetY);

				//if (((clip[targx][targetY]) & 0x20) == 0)
					//return true;
				System.out.println("k den " + ((clip[targx][targetY]) & 0x20));
				//System.out.println("passed?? " + srcEndX +", " + srcEndY + ", " + destEndX+", " + destEndY + ", " + currentX + ", " + currentY + ", " + sizeX + ", " + sizeY + ", " + targetX + ", " + targetY);
			}
		}*/
		return true;
	}
