package nl.bartpelle.veteres.model.map;
...
public class WalkRouteFinder {
	...
	public static int exitX = -1;
	public static int exitY = -1;
	 ...
	 */
	public static int findRoute(DefinitionRepository repo, int srcX, int srcY, int srcZ, int srcSizeXY, RouteStrategy strategy, boolean findAlternative, boolean duckmode) {
		isAlternative = false;
		...
		if (debug) {
			...
			transmitClipData(repo, srcX, srcY, srcZ);
			double debug_transmittime = System.nanoTime() - start;
		} else {
			transmitClipData(repo, srcX, srcY, srcZ);
		}
		...
		switch (srcSizeXY) {
			case 1:
				found = performCalculationS1(srcX, srcY, strategy, duckmode);
				break;
			case 2:
		...
		if (!found && findAlternative) {
			...
			int approxDestY = strategy.getApproxDestinationZ();
			...
			for (int checkX = (approxDestX - ALTERNATIVE_ROUTE_RANGE); checkX <= (approxDestX + ALTERNATIVE_ROUTE_RANGE); checkX++) {
				for (int checkY = (approxDestY - ALTERNATIVE_ROUTE_RANGE); checkY <= (approxDestY + ALTERNATIVE_ROUTE_RANGE); checkY++) {
					...
					if (approxDestY <= checkY) {
						deltaY = 1 - approxDestY - (strategy.getApproxDestinationSizeZ() - checkY);
						// deltaY = (approxDestY +
	 ...
	 */
	private static boolean performCalculationS1(int srcX, int srcY, RouteStrategy strategy, boolean duckmode) {
		boolean noduckmode = !duckmode;
		...
		while (read != write) {
			...
			if (currentGraphX > 0 && _directions[currentGraphX - 1][currentGraphY] == 0 && (_clip[currentGraphX - 1][currentGraphY] & ((noduckmode ? Flags.FLOOR_BLOCKSWALK : 0) | Flags.FLOORDECO_BLOCKSWALK | Flags.OBJ_BLOCKSWALK_ALTERNATIVE | Flags.WALLOBJ_EAST_BLOCKSWALK_ALTERNATIVE)) == 0) {
			...
			if (currentGraphX < (GRAPH_SIZE - 1) && _directions[currentGraphX + 1][currentGraphY] == 0 && (_clip[currentGraphX + 1][currentGraphY] & ((noduckmode ? Flags.FLOOR_BLOCKSWALK : 0) | Flags.FLOORDECO_BLOCKSWALK | Flags.OBJ_BLOCKSWALK_ALTERNATIVE | Flags.WALLOBJ_WEST_BLOCKSWALK_ALTERNATIVE)) == 0) {
			...
			if (currentGraphY > 0 && _directions[currentGraphX][currentGraphY - 1] == 0 && (_clip[currentGraphX][currentGraphY - 1] & ((noduckmode ? Flags.FLOOR_BLOCKSWALK : 0) | Flags.FLOORDECO_BLOCKSWALK | Flags.OBJ_BLOCKSWALK_ALTERNATIVE | Flags.WALLOBJ_NORTH_BLOCKSWALK_ALTERNATIVE)) == 0) {
			...
			if (currentGraphY < (GRAPH_SIZE - 1) && _directions[currentGraphX][currentGraphY + 1] == 0 && (_clip[currentGraphX][currentGraphY + 1] & ((noduckmode ? Flags.FLOOR_BLOCKSWALK : 0) | Flags.FLOORDECO_BLOCKSWALK | Flags.OBJ_BLOCKSWALK_ALTERNATIVE | Flags.WALLOBJ_SOUTH_BLOCKSWALK_ALTERNATIVE)) == 0) {
	 ...
	 */
	private static void transmitClipData(DefinitionRepository repo, int x, int y, int z) {
		...
		for (int transmitRegionX = graphBaseX >> 6; transmitRegionX <= (graphBaseX + (GRAPH_SIZE - 1)) >> 6; transmitRegionX++) {
			for (int transmitRegionY = graphBaseY >> 6; transmitRegionY <= (graphBaseY + (GRAPH_SIZE - 1)) >> 6; transmitRegionY++) {
				 ...
				 * !region.isLoadedObjectSpawns()) { for (int fillX = startX;
				 * fillX < endX; fillX++) for (int fillY = startY; fillY < endY;
				 * fillY++) clip[fillX - graphBaseX][fillY - graphBaseY] = -1; }
				 * else
				 */{
					MapDefinition mapdef = repo.get(MapDefinition.class, (transmitRegionX << 8) | transmitRegionY);
					if (mapdef != null) {
						int[][] masks = mapdef.masks[z];// map.getMasks()[z];
						for (int fillX = startX; fillX < endX; fillX++) {
							//System.arraycopy(masks[fillX & 0x3F], startY & 0x3F, clip[fillX - graphBaseX], startY - graphBaseY, (endY - startY) & 0x3F);
	 ...
	 */
	public static int[] getLastPathBufferZ() {
		return bufferY;
	}
	...
	public static boolean canGoWestFrom(int clip) {
		return (clip & (Flags.FLOOR_BLOCKSWALK | Flags.FLOORDECO_BLOCKSWALK | Flags.OBJ_BLOCKSWALK_ALTERNATIVE | Flags.WALLOBJ_WEST_BLOCKSWALK_ALTERNATIVE)) == 0;
	}

	public static boolean canGoEastFrom(int clip) {
		return (clip & (Flags.FLOOR_BLOCKSWALK | Flags.FLOORDECO_BLOCKSWALK | Flags.OBJ_BLOCKSWALK_ALTERNATIVE | Flags.WALLOBJ_EAST_BLOCKSWALK_ALTERNATIVE)) == 0;
	}

	public static boolean canGoNorthFrom(int clip) {
		return (clip & (Flags.FLOOR_BLOCKSWALK | Flags.FLOORDECO_BLOCKSWALK | Flags.OBJ_BLOCKSWALK_ALTERNATIVE | Flags.WALLOBJ_NORTH_BLOCKSWALK_ALTERNATIVE)) == 0;
	}

	public static boolean canGoSouthFrom(int clip) {
		return (clip & (Flags.FLOOR_BLOCKSWALK | Flags.FLOORDECO_BLOCKSWALK | Flags.OBJ_BLOCKSWALK_ALTERNATIVE | Flags.WALLOBJ_SOUTH_BLOCKSWALK_ALTERNATIVE)) == 0;
	}

	public static boolean canGoNorthEastFrom(int clipX, int clipY, int clipXY) {
		return (clipXY & (Flags.FLOOR_BLOCKSWALK | Flags.FLOORDECO_BLOCKSWALK | Flags.OBJ_BLOCKSWALK_ALTERNATIVE |
				Flags.WALLOBJ_SOUTH_BLOCKSWALK_ALTERNATIVE | Flags.WALLOBJ_WEST_BLOCKSWALK_ALTERNATIVE | Flags.CORNEROBJ_SOUTHWEST_BLOCKSWALK_ALTERNATIVE)) == 0 &&
				(clipX & (Flags.FLOOR_BLOCKSWALK | Flags.FLOORDECO_BLOCKSWALK | Flags.OBJ_BLOCKSWALK_ALTERNATIVE |
						Flags.WALLOBJ_WEST_BLOCKSWALK_ALTERNATIVE)) == 0 &&
				(clipY & (Flags.FLOOR_BLOCKSWALK | Flags.FLOORDECO_BLOCKSWALK | Flags.OBJ_BLOCKSWALK_ALTERNATIVE |
						Flags.WALLOBJ_SOUTH_BLOCKSWALK_ALTERNATIVE)) == 0;
	}

	public static boolean canGoNorthWestFrom(int clipX, int clipY, int clipXY) {
		return (clipXY & (Flags.FLOOR_BLOCKSWALK | Flags.FLOORDECO_BLOCKSWALK | Flags.OBJ_BLOCKSWALK_ALTERNATIVE |
				Flags.WALLOBJ_EAST_BLOCKSWALK_ALTERNATIVE | Flags.WALLOBJ_SOUTH_BLOCKSWALK_ALTERNATIVE | Flags.CORNEROBJ_SOUTHEAST_BLOCKSWALK_ALTERNATIVE)) == 0 &&
				(clipX & (Flags.FLOOR_BLOCKSWALK | Flags.FLOORDECO_BLOCKSWALK | Flags.OBJ_BLOCKSWALK_ALTERNATIVE |
						Flags.WALLOBJ_EAST_BLOCKSWALK_ALTERNATIVE)) == 0 && (clipY & (Flags.FLOOR_BLOCKSWALK | Flags.FLOORDECO_BLOCKSWALK |
				Flags.OBJ_BLOCKSWALK_ALTERNATIVE | Flags.WALLOBJ_SOUTH_BLOCKSWALK_ALTERNATIVE)) == 0;
	}

	public static boolean canGoSouthEastFrom(int clipX, int clipY, int clipXY) {
		return (clipXY & (Flags.FLOOR_BLOCKSWALK | Flags.FLOORDECO_BLOCKSWALK | Flags.OBJ_BLOCKSWALK_ALTERNATIVE |
				Flags.WALLOBJ_NORTH_BLOCKSWALK_ALTERNATIVE | Flags.WALLOBJ_WEST_BLOCKSWALK_ALTERNATIVE | Flags.CORNEROBJ_NORTHWEST_BLOCKSWALK_ALTERNATIVE)) == 0 &&
				(clipX & (Flags.FLOOR_BLOCKSWALK | Flags.FLOORDECO_BLOCKSWALK | Flags.OBJ_BLOCKSWALK_ALTERNATIVE |
						Flags.WALLOBJ_WEST_BLOCKSWALK_ALTERNATIVE)) == 0 && (clipY & (Flags.FLOOR_BLOCKSWALK | Flags.FLOORDECO_BLOCKSWALK |
				Flags.OBJ_BLOCKSWALK_ALTERNATIVE | Flags.WALLOBJ_NORTH_BLOCKSWALK_ALTERNATIVE)) == 0;
	}

	public static boolean canGoSouthWestFrom(int clipX, int clipY, int clipXY) {
		return (clipXY & (Flags.FLOOR_BLOCKSWALK | Flags.FLOORDECO_BLOCKSWALK | Flags.OBJ_BLOCKSWALK_ALTERNATIVE |
				Flags.WALLOBJ_NORTH_BLOCKSWALK_ALTERNATIVE | Flags.WALLOBJ_EAST_BLOCKSWALK_ALTERNATIVE | Flags.CORNEROBJ_NORTHEAST_BLOCKSWALK_ALTERNATIVE)) == 0 &&
				(clipX & (Flags.FLOOR_BLOCKSWALK | Flags.FLOORDECO_BLOCKSWALK | Flags.OBJ_BLOCKSWALK_ALTERNATIVE |
						Flags.WALLOBJ_EAST_BLOCKSWALK_ALTERNATIVE)) == 0 && (clipY & (Flags.FLOOR_BLOCKSWALK | Flags.FLOORDECO_BLOCKSWALK |
				Flags.OBJ_BLOCKSWALK_ALTERNATIVE | Flags.WALLOBJ_NORTH_BLOCKSWALK_ALTERNATIVE)) == 0;
	}

	public static boolean canShootWestFrom(int clip) {
		return (clip & (Flags.WALLOBJ_WEST_BLOCKSFLY | Flags.OBJ_BLOCKSFLY)) == 0;
	}

	public static boolean canShootEastFrom(int clip) {
		return (clip & (Flags.WALLOBJ_EAST_BLOCKSFLY | Flags.OBJ_BLOCKSFLY)) == 0;
	}

	public static boolean canShootNorthFrom(int clip) {
		return (clip & (Flags.WALLOBJ_NORTH_BLOCKSFLY | Flags.OBJ_BLOCKSFLY)) == 0;
	}

	public static boolean canShootSouthFrom(int clip) {
		return (clip & (Flags.WALLOBJ_SOUTH_BLOCKSFLY | Flags.OBJ_BLOCKSFLY)) == 0;
	}

	public static boolean canShootNorthEastFrom(int clipX, int clipY, int clipXY) {
		return (clipXY & (Flags.OBJ_BLOCKSFLY | Flags.WALLOBJ_SOUTH_BLOCKSFLY | Flags.WALLOBJ_WEST_BLOCKSFLY | Flags.CORNEROBJ_SOUTHWEST_BLOCKSFLY)) == 0 &&
				(clipX & (Flags.OBJ_BLOCKSFLY | Flags.WALLOBJ_WEST_BLOCKSFLY)) == 0 &&
				(clipY & (Flags.OBJ_BLOCKSFLY | Flags.WALLOBJ_SOUTH_BLOCKSFLY)) == 0;
	}

	public static boolean canShootNorthWestFrom(int clipX, int clipY, int clipXY) {
		return (clipXY & (Flags.OBJ_BLOCKSFLY | Flags.WALLOBJ_EAST_BLOCKSFLY | Flags.WALLOBJ_SOUTH_BLOCKSFLY | Flags.CORNEROBJ_SOUTHEAST_BLOCKSFLY)) == 0 &&
				(clipX & (Flags.OBJ_BLOCKSFLY | Flags.WALLOBJ_EAST_BLOCKSFLY)) == 0 &&
				(clipY & (Flags.OBJ_BLOCKSFLY | Flags.WALLOBJ_SOUTH_BLOCKSFLY)) == 0;
	}

	public static boolean canShootSouthEastFrom(int clipX, int clipY, int clipXY) {
		return (clipXY & (Flags.OBJ_BLOCKSFLY | Flags.WALLOBJ_NORTH_BLOCKSFLY | Flags.WALLOBJ_WEST_BLOCKSFLY | Flags.CORNEROBJ_NORTHWEST_BLOCKSFLY)) == 0 &&
				(clipX & (Flags.OBJ_BLOCKSFLY | Flags.WALLOBJ_WEST_BLOCKSFLY)) == 0 &&
				(clipY & (Flags.OBJ_BLOCKSFLY | Flags.WALLOBJ_NORTH_BLOCKSFLY)) == 0;
	}

	public static boolean canShootSouthWestFrom(int clipX, int clipY, int clipXY) {
		return (clipXY & (Flags.OBJ_BLOCKSFLY | Flags.WALLOBJ_NORTH_BLOCKSFLY | Flags.WALLOBJ_EAST_BLOCKSFLY | Flags.CORNEROBJ_NORTHEAST_BLOCKSFLY)) == 0 &&
				(clipX & (Flags.OBJ_BLOCKSFLY | Flags.WALLOBJ_EAST_BLOCKSFLY)) == 0 &&
				(clipY & (Flags.OBJ_BLOCKSFLY | Flags.WALLOBJ_NORTH_BLOCKSFLY)) == 0;
	}
