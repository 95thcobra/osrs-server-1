package nl.bartpelle.veteres.plugin;
...
public final class PluginHandler {
     ...
     */
    private final Logger logger = Logger.getLogger(PluginHandler.class.getSimpleName());
    ...
    public void init() {
        try {
            ...
            System.out.println("Loaded plugin bootstrap!");
        } catch (Exception e) {
            e.printStackTrace();
            //logger.log(Level.SEVERE, "An error has occured while initializing the Bootstrap!", e);
            System.out.println("An error has occured while initializing the Bootstrap!");
        }
    }
     ...
     * type.
     *
     * @param player  the player that these plugin listeners will be executed for.
     * @param type    the context type to grab the plugins listeners for.
     * @param context the plugin context that will supply data to the plugin
     *                listeners.
     */
     ...
     *
     * @param clazz the class of the plugin listener that will be submitted.
     */
